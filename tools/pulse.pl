...
my @pulse_tables = (
    gen_pulse_table2(1/2),
    gen_pulse_table2(1/4),
    gen_pulse_table2(1/8),
    gen_pulse_table2(1/16),
    gen_pulse_table2(1/32),
    gen_pulse_table2(1/64),
    gen_saw_table(),
    gen_tri_table()
);

draw_graph( 'pulse.png', \@pulse_tables );
...
sub gen_pulse_table {
    ...
    my @tmp = map {
        my $i = $_;
        my $val = (($i / $n) < 0.5) ? 1 : -1;
        my_round( $val * VALUE_MAX );
    } 0..($n - 1);

    return \@tmp;
}

sub gen_pulse_table2 {
    my $duty = $_[0] // 0.5;
    #say "duty = ${duty}";
    my $gain0 = sinc( $duty );

    my $n = 2 ** TABLE_SIZE_BITS;
    my @tmp = map $duty - 0.5, 0..($n - 1);
    ...
    for ( 0..511 ) {
        ...
        for (my $i=0; $i<$n; $i++) {
            $tmp[$i] += $gain * sin( $duty * $fn  * pi ) * cos( $fn * ($i / $n) * 2 * pi );
        }
    }
